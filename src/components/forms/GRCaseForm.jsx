...
const handlenextDateStatusChange = (index, value) => {
...
const addnextDateStatus = () => {
  setFormData({
    ...formData,
    nextDateStatus: [...formData.nextDateStatus, ""],
  });
};
...
const removenextDateStatus = (index) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await fetch(`${process.env.NEXT_PUBLIC_FETCH_API_TWO}`, {
        method: "POST",
        headers: {
...
<p className="mt-2 font-medium text-red-600">
  {duplicateMsg}
          ...
          <div className="w-16">
            <label className="label font-medium">মাস * </label>
            <select name="caseMonth" className="select select-bordered w-full" onChange={handleChange}>
  ...
  {formData.nextDateStatus.map((date, index) => (
    <div key={index} className="flex gap-2 mb-2">
      <input
        type="date"
        value={date}
        onChange={(e) => handlenextDateStatusChange(index, e.target.value)}
